...
## Step 3 — Per run: upload photos and fork
...
run = client.interactions.create(
    ...
    environment="remote",                  # fork the base env
    input=[
        {"type": "image", "file_uri": photo.uri, "mime_type": photo.mime_type},
        {"type": "text", "text": (
            "Save the attached photo to characters/Shane/source/shane1.jpg, then run, "
            "from /workspace/repo:\n"
            "  uv run scripts/prep_source_photos.py --character Shane --apply\n"
            "  uv run scripts/generate_character_sheets.py --character Shane --send "
            "--allow-proxy-auth --overwrite\n"
            "Report the path of the generated sheet."
        )},
    ],
    background=True,
)
# poll run.id until completed, then download the sheet from the sandbox
# (see the Managed Agents Quickstart for the file-download call).
```
